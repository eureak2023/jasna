...
(NVDEC) backend, which needs the custom PyAV (current_ctx API) that venv installs; set
JASNA_SIDECAR_DECODE=sw to fall back to stock-PyAV software decode, which is fast enough on its
own (~769 fps at 1080p — decode is never the bottleneck) but costs ~30% end to end here.
"""
...
# NOTE: forcing software decode used to need a monkeypatched NvidiaVideoReader.__enter__ here
# (stock PyAV, bypassing the custom PyAV's current_ctx requirement). jasna 0.10.0 has that
# natively as the "pyav-sw" decode backend, so run_sidecar() just selects it by env var -- one
# less reimplementation of an __enter__ that keeps growing state we would have to mirror.
...
# the pyav-sw backend forces BOTH the detection AND blend readers onto the SW->CUDA upload path
...
def build_pipeline(device_str, model_weights, max_clip_size, batch_size, temporal_overlap, fp16):
    ...
    with device_context(device):
        ...
        pipeline = Pipeline(
            ...
            # left ON: a cut inside a clip otherwise drags the previous shot's mosaic track
            # across it, and the detector runs per batch anyway so it costs us nothing here.
...
def run_pass(pipeline, device, metadata, gen, seek_ts, cancel_event):
    ...
    threads = [
        threading.Thread(target=lambda: decode_detect_loop(
            ...
            scene_detection=pipeline.scene_detection, blend_buffer=blend_buffer,
...
def run_sidecar():
    ...
    # Decode backend. Default "hw" -> jasna's pyav-hw (NVDEC) path, which needs the custom
    # PyAV's current_ctx. Measured +30% throughput at 1080p (50->65 fps) AND zero corruption --
    ...
    # fallback against the allocator aliasing. JASNA_SIDECAR_DECODE=sw forces software decode
    # for troubleshooting; any other value is passed through as a jasna backend name, so
    # JASNA_SIDECAR_DECODE=vali|auto also work. We pin pyav-hw rather than leaving jasna's own
    # "auto" default because auto probes VALI first, and without the corruption-tolerant
    # python_vali fork installed that is a failed open plus a warning on every file.
    _vd.new_stream = _default_stream               # SW-decode conversions share the default stream (see above)
    if not os.environ.get("JASNA_DECODE_BACKEND"):  # an explicit override always wins
        _sel = os.environ.get("JASNA_SIDECAR_DECODE", "hw").lower()
