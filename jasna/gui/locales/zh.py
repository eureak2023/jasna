...
ZH = {
        ...
        "image_restore_hint": "当输入为静态图片时自动使用：检测马赛克并用下方选择的引擎修复。视频输入会忽略这些设置。",
        "image_restore_model": "模型",
        "image_restore_engine": "引擎",
        "image_restore_clip_size": "片段长度",
        "image_restore_not_needed": "无需下载",
        ...
        "tip_image_restore_engine": "选择用于修复静态图片的模型。\n\nSD 1.5 用微调的扩散模型重绘马赛克：需要单独下载 6.9 GB 并具备赞助者许可，且会自行编造细节。\n\n视频模型直接复用 Jasna 自带的 BasicVSR++ 权重，在由该图片构成的静止片段上运行。无需下载、速度快得多，也更贴近原图；但单张图片没有额外信息，效果无法与视频相比。",
        "tip_image_restore_clip_size": "仅视频模型：合成片段中相同图片的张数，取中间那张作为结果。\n\n循环网络每一步都会细化特征，片段越长处理轮数越多。约 5 之后收益趋于平缓，因此默认值为 5。",
