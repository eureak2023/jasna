...
JA = {
        ...
        "image_restore_hint": "入力が静止画のときに自動的に使用されます。モザイクを検出し、下で選択したエンジンで復元します。動画入力ではこの設定は無視されます。",
        "image_restore_model": "モデル",
        "image_restore_engine": "エンジン",
        "image_restore_clip_size": "クリップ長",
        "image_restore_not_needed": "不要",
        ...
        "tip_image_restore_engine": "静止画を復元するモデルを選びます。\n\nSD 1.5 はファインチューニングした拡散モデルでモザイクを描き直します。別途 6.9 GB のダウンロードとサポーターライセンスが必要で、それらしいディテールを生成します。\n\n動画モデルは Jasna に同梱の BasicVSR++ の重みを再利用し、画像から作った静止クリップに対して実行します。追加ダウンロードは不要で高速、元画像に忠実ですが、1 枚には追加情報がないため動画ほどの効果は出ません。",
        "tip_image_restore_clip_size": "動画モデル専用: 合成クリップを構成する同一画像の枚数です。中央の 1 枚を採用します。\n\n再帰型ネットワークは各ステップで特徴を洗練するため、長いほど処理回数が増えます。5 前後で効果は頭打ちになるので既定は 5 です。",
