...
EN = {
        ...
        "image_restore_hint": "Used automatically when the input is a still image: mosaics are detected and restored with the engine selected below. Video inputs ignore these settings.",
        "image_restore_model": "Model",
        "image_restore_engine": "Engine",
        "image_restore_clip_size": "Clip size",
        "image_restore_not_needed": "Not needed",
        ...
        "tip_image_restore_engine": "Which model restores still images.\n\nSD 1.5 inpaints the mosaic with a fine-tuned diffusion model: a separate 6.9 GB download that needs a supporter licence, and it invents plausible detail.\n\nVideo model reuses the BasicVSR++ weights Jasna already ships, run on a static clip built from the image. Nothing to download, much faster, and it stays closer to the source - but a single frame carries no extra information, so it cannot match what it does on video.",
        "tip_image_restore_clip_size": "Video model only: how many copies of the image make up the synthetic clip; the middle one is kept.\n\nThe recurrent network refines its features at every step, so a longer clip means a few more passes. Gains flatten out past about 5, which is the default.",
