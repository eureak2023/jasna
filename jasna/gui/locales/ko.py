...
KO = {
        ...
        "image_restore_hint": "입력이 정지 이미지일 때 자동으로 사용됩니다. 모자이크를 감지하고 아래에서 선택한 엔진으로 복원합니다. 동영상 입력에서는 이 설정이 무시됩니다.",
        "image_restore_model": "모델",
        "image_restore_engine": "엔진",
        "image_restore_clip_size": "클립 길이",
        "image_restore_not_needed": "필요 없음",
        ...
        "tip_image_restore_engine": "정지 이미지를 복원할 모델을 선택합니다.\n\nSD 1.5는 미세 조정된 디퓨전 모델로 모자이크를 새로 그립니다. 별도 6.9 GB 다운로드와 서포터 라이선스가 필요하고, 그럴듯한 디테일을 만들어냅니다.\n\n동영상 모델은 Jasna에 이미 들어 있는 BasicVSR++ 가중치를 그대로 써서, 이미지로 만든 정지 클립에 적용합니다. 추가 다운로드가 없고 훨씬 빠르며 원본에 더 충실하지만, 한 장에는 추가 정보가 없으므로 동영상만큼의 복원력은 나오지 않습니다.",
        "tip_image_restore_clip_size": "동영상 모델 전용: 합성 클립을 이루는 동일 이미지의 장수이며, 가운데 한 장을 사용합니다.\n\n순환 신경망은 매 단계마다 특징을 다듬으므로 길수록 패스가 늘어납니다. 5 부근에서 효과가 포화되어 기본값은 5입니다.",
