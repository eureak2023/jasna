...
def classify_folder(folder: str | Path, recursive: bool = False) -> tuple[list[Path], list[Path]]:
    """Return ``(images, videos)`` — sorted media files in ``folder``.

    Top level only by default. With ``recursive``, walks the whole tree and
    orders shallowest-first (the same key ``folder_media_in_processing_order``
    uses), so a parent folder covers every subfolder below it.
    """
    folder = Path(folder)
    if recursive:
...
def folder_output_path(
    ...
    ``input_root`` is the folder the batch was collected from. Passing it mirrors
    the input's subfolder under ``output_dir`` — required for recursive batches,
    where two subfolders may hold the same filename (and where in-place means
    "back into the same subfolder"). Without it the output is flat, as before.
    """
    input_path = Path(input_path)
    sub_dir = Path()
    if input_root is not None:
        try:
            sub_dir = input_path.relative_to(Path(input_root)).parent
        except ValueError:
            sub_dir = Path()
    base_dir = Path(output_dir) / sub_dir

    if not output_pattern:
        return base_dir / f"{input_path.stem}_out{input_path.suffix}"
