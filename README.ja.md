...
GUI のすべての設定にはツールチップがあります — 横の ⓘ アイコンにマウスを合わせてください。キュー全体の再実行や並べ替え、プリセット、出力テンプレートなどの残りの機能は [GUI ガイド](docs/ja/gui.md)で紹介しています。
...
- **[高度な処理](docs/ja/advanced_processing.md)** — ノイズ除去、60→30 FPS 書き出し、カラー LUT、シャープネス、エンコーダーへそのまま渡す CQ 品質設定、カスタムエンコーダー設定、エクスポート後のアクション。
