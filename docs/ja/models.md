...
静止画では、Jasna は動画パイプラインを使いません。GUI のキューに画像を追加する
（または CLI で渡す）だけで、画像ジョブは自動的に静止画パスへルーティング
されます:
...
エンジンは 2 種類あり、`--image-restoration-model-name`（GUI の画像復元セクションの
**エンジン**）で選びます:

| エンジン | 必要なもの | 方式 |
| ------ | ----- | -------- |
| `basicvsrpp`（既定） | 追加なし | 画像から作った静止クリップに動画復元モデルを実行する。 |
| `sd-15-jav` | 6.9 GB のダウンロード + サポーターライセンス | ファインチューニング済みの拡散モデルでモザイクを描き直す（それらしいディテールを生成）。 |
...
検出・クロップ形状・ブレンドは動画パイプラインとまったく同じで、クリップだけが合成です
— 同一画像を `--image-clip-size` 枚（既定 5）並べ、中央の復元フレームを採用します。
`model_weights/` にある重みを再利用するため、ダウンロードもライセンス入力も不要で、
1 枚あたり 1 秒未満で完了します。

静止画には融合できる隣接フレームがないため、これは BasicVSR++ が動画で達成する復元
というより、その 1 枚に対する学習済みモザイク除去に近いものです。その代わり、新しい
内容を生成せず元画像に忠実です。`--image-clip-size` を上げると再帰ネットワークの
リファイン回数が増えますが、5 前後で差はほぼなくなります。
